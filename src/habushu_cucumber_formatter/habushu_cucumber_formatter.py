...
class CucumberJSONFormatter(Formatter):
    ...
    json_scalar_types = six.integer_types + (float,) + (six.text_type, bool, type(None))
    ...
    def make_table(cls, table):
        return {"headings": table.headings, "rows": [list(row) for row in table.rows]}
    ...
    def write_tags(self, tags):
        return [
            {"name": f"@{tag}", "line": tag.line if hasattr(tag, "line") else 1}
            for tag in tags
        ]
...
class PrettyCucumberJSONFormatter(CucumberJSONFormatter):
    ...
    description = "Cucumber JSON dump of test run (human readable)"
